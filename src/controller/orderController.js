...
const createOrder = async (req, res) => {
    try {
        ...
        if (!validator.isValidRequest(cartBody)) return res.status(400).send({ status: false, message: "Please provide body" });

        if (!(validator.isValidObjectId(cartId))) return res.status(400).send({ status: false, message: "Provide a valid cartId" });
        ...
        if (!checkCart) return res.status(404).send({ status: false, message: "Cart doesn't exist" })
        if (!checkCart.items.length) return res.status(400).send({ status: false, message: "Please add some product in cart to make an order" });
        ...
        if (status == 'pending' || createOrder.status == 'pending') {
            await cartModel.findOneAndUpdate({ _id: cartId, userId: userId },{$set:{items: [], totalPrice: 0, 
                totalItems: 0, }})
        };
...
const updateOrder = async (req, res) => {
    try {
        ...
        if (!validator.isValidRequest(req.body)) return res.status(400).send({ status: false, message: "Please provide body" });
        
        if (!(validator.isValidObjectId(userId))) return res.status(400).send({ status: false, message: "Provide a valid userId" });


        if (!(validator.isValidObjectId(orderId))) return res.status(400).send({ status: false, message: "Provide a valid orderId" });

        if (!validator.validStatus(status)) return res.status(400).send({ status: false, message: "Status can be completed or cancelled only" });

        if (status == 'pending') return res.status(400).send({ status: false, message: "status can not be pending during updation" })
        ...
        if (status == 'completed' && (findOrder.status == 'completed' || findOrder.status == 'cancelled')) {
            return res.status(400).send({ status: false, 
                message: `Order status can not be changed after ${order.status}` })
        }
        ...
        if(status=="cancelled" && findOrder.cancellable !== true) return res.status(200).send({ status: false, message: "Order can not be cancelled" })
